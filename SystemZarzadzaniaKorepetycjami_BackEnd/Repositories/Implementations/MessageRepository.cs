...
public class MessageRepository : IMessageRepository
{
    ...
    public async Task<List<ConversationDTO>> GetConversationsByPersonAsync(Person user)
    {
    ...
    public async Task<List<MessageDTO>> GetConversationBySenderAndReciverAsync(Person user, Person corespondent)
    {
        var messages = await _context.Message
            .Where(m => m.Sender == user.IdPerson || m.Receiver == corespondent.IdPerson ||
                        m.Sender == corespondent.IdPerson || m.Receiver == user.IdPerson)
            .OrderByDescending(m => m.Date)
            .Select(m => new MessageDTO
            {
                SenderId = m.Sender,
                ReceiverId = m.Receiver,
                Date = m.Date.ToString("yyyy-MM-dd HH:mm", new CultureInfo("pl-Pl")),
                Content = m.Content
            })
            .ToListAsync();
        return messages;
