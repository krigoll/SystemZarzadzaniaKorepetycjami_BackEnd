...
using SystemZarzadzaniaKorepetycjami_BackEnd.Repositories.Interfaces;
    ...
    public class CalendarRepository : ICalendarRepository
    {
        ...
        public async Task<List<List<CalendarDTO>>> GetCalendarsByPersonAndRangeFromToAsync(Person person, DateTime from, DateTime to)
        {
            ...
            for (var i = from; i <= to; i = i.AddDays(1))
            {
                var calendarsOfTheDay = await (from lesson in _context.Lesson
                    ...
                    where (lesson.IdTeacher == person.IdPerson || lesson.IdStudent == person.IdPerson) && lesson.StartDate == i
