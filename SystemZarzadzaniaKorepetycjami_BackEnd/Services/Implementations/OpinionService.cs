...
public class OpinionService
{
    ...
    public async Task<List<OpinionDTO>> GetOpinionsByStudentEmailAsync(string email)
    {
        ...
        var opinions = await _opinionRepository.GetOpinionsByStudentAsync(student);
        return opinions;
    }
    ...
    public async Task<List<OpinionDTO>> GetOpinionsByTeacherIdAsync(int teacherId)
    {
        ...
        var opinions = await _opinionRepository.GetOpinionsByTeacherAsync(teacher);
        return opinions;
    }
    ...
    public async Task<CreateOpinionStatus> CreateOpinionAsync(OpinionCreateDTO opinionCreateDTO)
    {
        try
        {
            var student = await _studentRepository.GetStudentByEmailAsync(opinionCreateDTO.StudentEmail);
            var teacher = await _teacherRepository.GetTeacherByIdAsync(opinionCreateDTO.TeacherId);
            if (student == null || teacher == null) return CreateOpinionStatus.INVALID_STUDENT_OR_TEACHER;

            /*if (!(await _lessonRepository.AreThisTeacherTeachThisStudentAsync(teacher, student))) //TODO
            {
                return CreateOpinionStatus.CAN_NOT_OPINION_TEACHER_WITCH_NOT_TEACHED_YOU;
            }*/
            ...
            return CreateOpinionStatus.OK;
        }
        catch (ArgumentException e)
        {
            Console.WriteLine(e);
            return CreateOpinionStatus.INVALID_OPINION;
        }
        catch (Exception e)
        {
            Console.WriteLine(e);
            return CreateOpinionStatus.SERVER_ERROR;
        }
    }

    public async Task<CreateOpinionStatus> UpdateOpinionByIdAsync(int opinionId, OpinionCreateDTO opinionCreateDTO)
    {
        try
        {
            var student = await _studentRepository.GetStudentByEmailAsync(opinionCreateDTO.StudentEmail);
